...
export const SettingsRoute: React.FC = () => {
    try {
        const {settings, updateSetting, logout, updateSubkeySetting} = useContext(AppContext);
        const history = useHistory();

        ipcRenderer.on(
            'update-native-theme',
            (_, updatedAppearance: Appearance) => {
            ...
            <div className="flex flex-1 flex-col dark:bg-gray-dark dark:text-white">
                ...
                <div className="flex-1 px-8">
                    ...
                    <div className="flex">
                        ...
                        <FieldCheckbox name="onMergedFilter" label="Merged"
                                       checked={settings.excludedNotificationsTypes.merged}
                                       onChange={(evt) => {
                                           updateSubkeySetting('excludedNotificationsTypes', 'merged', evt.target.checked)
                                       }}/>
                        <FieldCheckbox name="onAssignedFilter" label="Assigned"
                                       checked={settings.excludedNotificationsTypes.assigned}
                                       onChange={(evt) => {
                                           updateSubkeySetting('excludedNotificationsTypes', 'assigned', evt.target.checked)
                                       }}/>

                    </div>
                    <div id="excludeDropdown" className='flex'>
                        {Reasons.each{(opt) => }
                        }
                    </div>
                </div>
        ...
        console.log('Unable to render settings');
    }
};
