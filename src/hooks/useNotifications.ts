...
export const useNotifications = (): NotificationsState => {
    ...
    const fetchNotifications = useCallback(
        async (accounts: AuthState, settings) => {
            ...
            async function getGitHubNotifications(page = 0): Promise<AxiosResponse> {
                if (!isGitHubLoggedIn) {
                    return;
                }
            ...
            async function getAllGithubNotifications(): Promise<AxiosResponse<any>[]> {
                return new Promise(async (result) => {
                    ...
                    do {
                        ...
                        const resp: AxiosResponse = await getGitHubNotifications(page);
                        if (resp.status > 299) {
                            throw new Error(`Github.com server connect error ${resp.status} ${resp.statusText}`)
                        }
                        if (resp.data.length > 0) {
                            responses.push(resp)
                        }

                        lastResponseData = resp.data;
                    } while (lastResponseData.length == 50)
            ...
            const promise = new Promise<void>(async () => {
                ...
                const gitResponses = await getAllGithubNotifications();
